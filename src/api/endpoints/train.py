...
def register_endpoints(api, train_service):
    ...
    # 모델 정의
    training_data_config = train_ns.model(
        "TrainingDataConfig",
        {
            "use_validation": fields.Boolean(
                description="검증 데이터 사용 여부", default=True
            ),
            "validation_split": fields.Float(
                description="검증 데이터 비율", default=0.2
            ),
            "random_seed": fields.Integer(description="랜덤 시드", default=42),
        },
    )

    parameters_model = train_ns.model(
        "Parameters",
        {
            "embedding_size": fields.Integer(
                description="임베딩 차원 수", required=True
            ),
    ...
    training_request = train_ns.model(
        "TrainingRequest",
        {
            "model_name": fields.String(description="모델 이름", required=True),
            "model_type": fields.String(
                description="모델 유형(ncf, feature_based, content_based, hybrid)",
                required=True,
            ),
            "parameters": fields.Nested(
                parameters_model, description="모델 학습 파라미터", required=True
            ),
            "training_data_config": fields.Nested(
                training_data_config, description="학습 데이터 설정"
            ),
        },
    )

    training_response = train_ns.model(
        "TrainingResponse",
        {
            "training_id": fields.String(description="학습 ID"),
            "status": fields.String(
                description="학습 상태(queued, running, completed, failed)"
            ),
    ...
    test_data_config = train_ns.model(
        "TestDataConfig",
        {
            "test_size": fields.Float(description="테스트 데이터 비율", required=True),
            "metrics": fields.List(
                fields.String, description="검증할 메트릭 목록", required=True
            ),
        },
    )

    validation_request = train_ns.model(
        "ValidationRequest",
        {
            "model_id": fields.String(description="검증할 모델 ID", required=True),
            "test_data_config": fields.Nested(
                test_data_config, description="테스트 데이터 설정", required=True
            ),
        },
    )

    validation_metrics = train_ns.model(
        "ValidationMetrics",
        {
            ...
            "ndcg@10": fields.Float(
                description="Normalized Discounted Cumulative Gain"
            ),
        },
    )

    validation_response = train_ns.model(
        "ValidationResponse",
        {
            ...
            "metrics": fields.Nested(
                validation_metrics, description="검증 메트릭 결과"
            ),
            "validation_plots": fields.List(
                fields.String, description="검증 결과 시각화 URL"
            ),
        },
    )
