...
def register_endpoints(api, recommendation_service):
    ...
    user_profile = api.model('UserProfile', {
        ...
        'preferred_activities': fields.List(fields.String, description='선호 활동 코드 목록'),
        'preferred_transport': fields.List(fields.String, description='선호 이동수단 목록')
    })
    
    # 도시 추천 모델 정의
    city_request = api.model('CityRecommendationRequest', {
        ...
        'recommendation_type': fields.String(description='추천 유형(place, travel, both)', default='both'),
        'alpha': fields.Float(description='추천 알고리즘 가중치(0-1)', default=0.7)
    })
    
    city_item = api.model('CityRecommendationItem', {
    ...
    city_response = api.model('CityRecommendationResponse', {
        'user_id': fields.String(description='사용자 ID'),
        'recommendations': fields.List(fields.Nested(city_item), description='추천 목록')
    })
    ...
    recommendation_details = api.model('RecommendationDetails', {
        ...
        'related_transport': fields.List(fields.String, description='연관 이동수단')
    })
    ...
    recommendation_item = api.model('RecommendationItem', {
        ...
        'details': fields.Nested(recommendation_details, description='추천 상세 정보')
    })
    
    recommendation_response = api.model('RecommendationResponse', {
        'user_id': fields.String(description='사용자 ID'),
        'recommendations': fields.List(fields.Nested(recommendation_item), description='추천 목록')
    })
    ...
    class CityRecommendationResource(Resource):
        ...
        def get(self):
            ...
            Returns:
                도시 추천 목록
            """
            return recommendation_service.get_city_recommendations(city_recommendation_ns.payload)
