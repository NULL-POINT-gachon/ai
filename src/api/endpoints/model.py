...
incident_ns = Namespace('recommendation/incidents', description='AI 여행지 추천 장애기록 API')
...
def register_model_endpoints(api, model_service):
    ...
    models_response = model_ns.model('ModelsResponse', {
    ...
    parameters_update = model_ns.model('ParametersUpdate', {
        'parameters': fields.Raw(description='수정할 파라미터', required=True)
    })
    
    parameters_response = model_ns.model('ParametersResponse', {
        'status': fields.String(description='작업 상태'),
        'model_id': fields.String(description='수정된 모델 ID'),
        'updated_parameters': fields.List(fields.String, description='수정된 파라미터 목록')
    })
    ...
    class ModelsResource(Resource):
        ...
        @model_ns.doc(description='여행지 추천 모듈 버전관리 기능',
                     params={
...
def register_incident_endpoints(api, incident_service):
    ...
    # 모델 정의
    error_details = incident_ns.model('ErrorDetails', {
    ...
    incident_request = incident_ns.model('IncidentRequest', {
        'severity': fields.String(description='심각도(high, medium, low)', required=True),
        'component': fields.String(description='장애 발생 컴포넌트', required=True),
        'description': fields.String(description='장애 설명', required=True),
        'error_details': fields.Nested(error_details, description='오류 세부 정보', required=True)
    })
    
    incident_response = incident_ns.model('IncidentResponse', {
